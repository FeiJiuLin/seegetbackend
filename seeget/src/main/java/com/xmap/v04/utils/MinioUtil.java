...
import com.xmap.v04.config.MinioConfig;
import com.xmap.v04.controller.UserController;
...
public class MinioUtil {
    @Autowired
    private MinioClient minioClient;// = new MinioConfig().getMinioClient();//初始化minnio
    ...
    public Boolean createBucket(String bucket) throws Exception {
        boolean exists = existBucket(bucket);//校验桶是否存在
        if (!exists){
            try {
